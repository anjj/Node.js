/*
    Autor: Troskia
    proyect: Wine REST Api
    Description: Wine REST Api for simple use 
*/
...
db.open(function (err, db) {
    if (!err) {
        ...
        db.collection('wines', { strict: true }, function (err, collection) {
            if (err) {
                console.log("The 'wines' collection dosen't exist, Creating it with sample data...");
                populateDB();
            }
        });
    }
...
exports.addWine = function (req, res) {
    ...
    db.collection('wines', function (err, collection) {
        collection.insert(wine, { safe: true }, function (err, result) {
            if (err) {
                res.send({ 'error': 'An error has analy ocurred' });
            } else {
...
exports.deleteWine = function (req, res) {
    ...
    db.collection('wines', function (err, collection) {
        collection.remove({ '_id': new BSON.ObjectID(id) }, { safe: true }, function (err, result) {
            if (err) {
                res.send({ 'error': 'An error ocurred' + err });
            } else {
                console.log('' + result + 'document(s) updated');
                res.send(req.body);
            }
